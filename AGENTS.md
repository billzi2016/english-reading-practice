...
- 文章必须有趣、新奇，优先写生活中不常见但真实存在的知识、人物选择、技术细节或历史瞬间。
- 文章要有清楚的故事线、具体场景和因果转折；但不得为了戏剧性编造事实或夸大结论。
- 词汇标注不能流于形式；难词、长术语、专业名词、材料名、地点名、历史概念和关键短语都要主动用中文括注帮助读者读懂。
- 推荐有趣题材包括：尼斯湖水怪、黑洞、引力波、斯特林发动机、拉瓦尔喷管、翼梢小翼、富士山旅游、阿尔罕布拉宫、Centralia 地下煤火、奇特乐器、音乐家、密码学、古代工程、罕见自然现象、深海探索、火山城市、失落文字、地图史、医学偶然发现、航空航天冷知识等。
...
- 文章不得包含歧视、仇恨、羞辱或鼓励排斥任何群体的表达；敏感主题必须保持事实、克制、学习性和尊重。
...
- 生成脚本固定使用 Ollama Python 库和 GPT-OSS 120B，默认 `think="medium"`；需要更稳但更慢时，只切换 `llm/ollama_generate.py` 的 `THINK_LEVEL` 为 `high`。正文生成 `temperature=0.1`，自动选题 `temperature=0.7`，除非用户明确要求不要改默认值。
